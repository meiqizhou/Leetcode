...
public class Solution {
    ...
    public int strobogrammaticInRange(String low, String high) {
    	...
    	long start = Long.parseLong(low), end = Long.parseLong(high);
    	for(int i = len1; i <= len2; i++) {
    		List<String> temp = findStrobogrammatic(i);
    		for(int j = 0; j < temp.size(); j++) {
    			if(Long.parseLong(temp.get(j)) >= start && Long.parseLong(temp.get(j)) <= end) count++;
    		}
    	}
    	return count;
    }
